...
Les `Image(systemName:)` ne se résolvent pas dans `ImageRenderer` : les boutons
apparaissent comme des carrés jaunes barrés. **C'est un artefact du rendu hors
écran, pas un bug de la vue.** Tout le reste est fidèle. Ne pas « corriger » des
icônes sur la foi de ces images.
