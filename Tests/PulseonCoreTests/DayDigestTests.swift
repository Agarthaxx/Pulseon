...
func sessionCrossingMidnightIsClamped() {
    ...
    let digest = builder.build(
        day: date(14, 12), sessions: [session], now: date(14, 23)
    )
...
func openSessionIsBoundedToNow() {
    ...
    let digest = builder.build(
        day: date(14, 12), sessions: [session], now: date(14, 11)
    )
...
func coveredTotalMergesOverlap() {
    ...
    let digest = builder.build(
        day: date(14, 12), sessions: sessions, now: date(15, 0)
    )
...
func laneTotalMergesItsOwnOverlaps() throws {
    ...
    let digest = builder.build(
        day: date(14, 12), sessions: sessions, now: date(14, 10)
    )
...
func entitiesAreRankedByTime() throws {
    ...
    let digest = builder.build(
        day: date(14, 12), sessions: sessions, now: date(14, 23)
    )
